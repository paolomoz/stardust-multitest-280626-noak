...
async function main() {
  ...
  try {
    const presp = await probe.goto(args.url, { waitUntil: 'domcontentloaded', timeout: 30000 });
    // Akamai/enterprise WAFs return a clean HTTP 403/429/503 to headless chromium —
    // goto does NOT throw on 4xx, so treat a blocked status as a fingerprint block.
    if (presp && [403, 429, 503].includes(presp.status())) {
      throw new Error(`net::ERR bot-management status ${presp.status()}`);
    }
  } catch (err) {
